...
First, the number of quantization levels is defined. This is determined by the number of bits used in the quantizer. The number of bits would also determine the resolution of the quantization. The formula to calculate the number of levels \(L\) is:  $ L = 2^{\text{No of bits}} $
...
Next, we determine the length/range of the signal. This can also be obtained through the formula:  $ R = y_{\max} - y_{\min} $
